...
function Video(editor) {
    this.editor = editor;
    var videoMenuId = getRandom('w-e-play');
    this.$elem = $('<div class="w-e-menu" id="' + videoMenuId + '"><i class="w-e-icon-play"><i/></div>');
...
Video.prototype = {
    ...
    _createInsertPanel: function _createInsertPanel() {
        ...
        var tabsConfigResult = [];
        if ((config.uploadVideoServer || config.customUploadVideo) && window.FileReader) {
            // 显示“上传视频”
            tabsConfigResult.push(tabsConfig[0]);
        }
...
UploadVideo.prototype = {
    ...
    uploadVideo: function uploadVideo(files) {
        ...
        var editor = this.editor;
        var config = editor.config;
        var uploadVideoServer = config.uploadVideoServer;//上传地址
                ...
                if (xhr.readyState === 4) {
                    ...
                    if (!hooks.customInsert && result.errno != '0') {
                        ...
                        if (hooks.customInsert && typeof hooks.customInsert === 'function') {
                            ...
                            console.log(data);
                            // data.forEach(function (link) {
                            //     console.log('lick:', link);
                            //     _this3.insertLinkVideo(link);
                            // });
                            _this3.insertLinkVideo(data.data[0]);
                        }
